"""
    ...
    By using this range, there are only a finite number of numbers that we are searching over, as opposed to the
    infinite number of decimals between 0 and 1. This range will help prevent infinite loops. The reason we
    use 0 to 10000 is to account for two additional decimal places in the range 0% to 100%.
    ...
    reach your desired down payment.
...
def calculate_savings_rate(starting_salary):
    ...
    # Bisection search initial settings
    low = 0
    high = 10000
    steps = 0
...
class TestCalculateSavingsRate(unittest.TestCase):
    ...
    def test_calculate_savings_rate_one(self):
        self.assertEqual(calculate_savings_rate(150000), 0.4409)

    def test_calculate_savings_rate_two(self):
        self.assertEqual(calculate_savings_rate(300000), 0.2205)

    # def test_calculate_savings_rate_three(self):
    #     self.assertEqual(calculate_savings_rate(10000), ("It is not possible to save for the down payment in 36 months.",))
