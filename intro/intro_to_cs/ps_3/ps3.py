...
def get_word_score(word, n):
    ...
    # for char in word:
    #     first_component += SCRABBLE_LETTER_VALUES.get(char.lower(), 0)
    first_component = sum(SCRABBLE_LETTER_VALUES[i] for i in word.lower())

    # val = 7 * wordlen - 3 * (n - wordlen)
    # second_component += val if val > 1 else 1
    ...
    while calculate_handlen(hand) > 0:
        ...
        if user_input == "!!":
            ...
            if is_valid_word(user_input, hand, word_list):
                ...
                total_score += score
                print(
                    f'"{user_input}" earned {score} points. Total: {total_score} points'
                )
            else:
...
def substitute_hand(hand, letter):
    ...
    """
    if letter in hand:
        new_letter = random.choice(
            [char for char in (VOWELS + CONSONANTS) if char not in hand]
        )
