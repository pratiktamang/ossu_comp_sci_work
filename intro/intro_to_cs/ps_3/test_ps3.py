...
def test_get_word_score():
    ...
    for word, n in words.keys():
        ...
        if score != words[(word, n)]:
            print_red("FAILURE: test_get_word_score()")
            print(
                "\tExpected",
                words[(word, n)],
                "points but got '"
            ...
            failure = True
    if not failure:
        print_green("SUCCESS: test_get_word_score()")
...
def test_update_hand():
    ...
    if hand2 != expected_hand1 and hand2 != expected_hand2:
        print_red("FAILURE: test_update_hand('" + word + "', " + str(handOrig) + ")")
        print(
            "\tReturned: ",
            hand2,
            "\n\t-- but expected:",
    ...
    print_green("SUCCESS: test_update_hand()")
...
def test_is_valid_word(word_list):
    ...
    if not is_valid_word(word, handCopy, word_list):
        print_red("FAILURE: test_is_valid_word()")
        print(
    ...
    if not failure:
        print_green("SUCCESS: test_is_valid_word()")
...
def test_wildcard(word_list):
    ...
    if is_valid_word(word, hand, word_list):
        print_red("FAILURE: test_is_valid_word() with wildcards")
        print("\tExpected False, but got True for word: '" + word + "' and hand:", hand)
    ...
    for word, n in words.keys():
        score = get_word_score(word, n)
        if score != words[(word, n)]:
            print_red("FAILURE: test_get_word_score() with wildcards")
            print(
                "\tExpected",
                words[(word, n)],
                "points but got '"
    ...
    if not failure:
        print_green("SUCCESS: test_wildcard()")
